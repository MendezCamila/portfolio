...
const ImageCarousel: React.FC<{
  ...
  className?: string;
  interval?: number;
}> = ({ images, image, alt, className = '', interval = 2500 }) => {
  const srcs = images && images.length > 0 ? images : image ? [image] : [];
  const [emblaRef, emblaApi] = useEmblaCarousel({ loop: true });

  useEffect(() => {
    if (!emblaApi || srcs.length <= 1) return;
    // preload
  ...
  }, [emblaApi, JSON.stringify(srcs), interval]);

  if (srcs.length === 0) return null;

  return (
    <div ref={emblaRef} className={`${className} overflow-hidden`} aria-label={alt || ''}>
      <div className="flex w-full h-full">
        {srcs.map((s, i) => (
          <div key={s + i} className="flex-none w-full h-full bg-center bg-cover" style={{ backgroundImage: `url(${s})` }} />
        ))}
      </div>
    </div>
  );
...
const ProjectCard: React.FC<ProjectCardProps> = ({ 
  ...
  const { t } = useLanguage();
  // use ImageCarousel (module-level) for image rotation
  return (
    <motion.div
  ...
  const projects: Project[] = [
    {
      ...
      description: t('projects.elisa.desc'),
      image: '/carrito.PNG',
      images: carouselMap[1],
      technologies: ['PHP', 'Laravel', 'MySQL', 'Livewire'],
      links: {
        code: 'https://github.com/MendezCamila/Elisa-Modas',
        demo: 'https://youtu.be/OLpNcpCuW4A?si=QdkAN6BgwNgiK0M4'
      },
      categories: ['business']
    },
    {
      ...
      description: t('projects.services.desc'),
      image: '/facturacion.PNG',
      images: carouselMap[2],
      technologies: ['Java', 'Spring Boot', 'MySQL'],
      links: { code: 'https://github.com/MendezCamila/GestionServicios' },
      categories: ['business']
    },
    {
      id: 3,
      title: t('projects.simulation.title'),
      description: t('projects.simulation.desc'),
      image: '/chicuadrado.png',
      images: carouselMap[3],
      technologies: ['Astro', 'Starlight', 'React', 'Supabase'],
      links: { code: 'https://gitlab.com/modysim8/ModeloSimulacion' },
      categories: ['business']
    },
    // (DocMind removed)
  ];
    ...
    return (
      <section id="projects" className="py-24 bg-gradient-to-b from-white to-gray-50">
        <div className="container mx-auto px-6">
          ...
          >
            <a 
              href="https://github.com/MendezCamila" 
              target="_blank"
              rel="noopener noreferrer"
